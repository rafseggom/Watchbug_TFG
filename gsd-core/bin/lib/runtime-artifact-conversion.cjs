...
function convertClaudeToCursorMarkdown(content) {
    ...
    // Replace "Cline" brand references with "Cursor"
...
/**
 * Convert a Cline command to a Cursor 1.6 slash command (#785).
 *
 ...
 *
 * @param {string} content   raw Cline command markdown (may have frontmatter)
...
function convertClaudeToWindsurfMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Cline bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Cline" brand references with "Windsurf"
...
function convertClaudeToAugmentMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Cline bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Cline" brand references with "Augment"
 ...
 * GSD's worktree isolation uses Cline's isolation="worktree" spawn
    ...
    switch (runtime) {
        ...
        case 'qwen': {
            ...
            if (_b) {
                ...
                content = content.replace(/\bClaude Code\b/g, _b['Cline']);
            }
            content = content.replace(/~\/\.claude\//g, pathPrefix);
            content = content.replace(/\$HOME\/\.claude\//g, pathPrefix);
